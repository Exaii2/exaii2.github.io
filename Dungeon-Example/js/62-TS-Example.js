...
var playerXP; // Stellt die gesammelte Erfahrung des Spielers dar.
...
var prefix = ["Wald-", "Seuchen-", "Uralte(s) ", "Gift-", "Brennende(s) ", "Kniescheibenzertrümmernde(s) "]; // length = 6, da 6 Einträge. Von 0-5.
var monsterName = ["Ratte", "Nagetier", "Ungeziefer"]; // length = 3, da 3 Einträge. Von 0-2.
var suffix = [" des Verderbens", " aus der Hölle", " der Lethalität", " mit Rheuma", " der Redundanz", " der Zerberstung"]; // length = 6, da hier 6 Einträge sind. Von 0-5.
...
function monsterGenerateHTML() {
    ...
    var monsterExp = document.createElement("p"); // Generiere einen <p>
    monsterExp.innerHTML = "Erfahrung: " + monsterArray[monsterArray.length - 1].monsterExperience; // Inhalt des <p>: Monster-Modifizierer null und eins
    holdingDiv.appendChild(monsterExp); // Füge das <p> zum HTML-Dokument hinzu, indem es dem holding-Div angefügt wird.
...
function generateMonsterModifer() {
    ...
    tempMonsterMod[0] = monsterModifers[getRNGNumber(monsterModifers.length)]; // Setze Schublade 0 des Arrays auf einen Wert, welcher durch Zufall auf den Wert des Arrays gesetzt wird.
    tempMonsterMod[1] = monsterModifers[getRNGNumber(monsterModifers.length)]; // Setze Schublade 1 des Arrays auf einen Wert.
    return tempMonsterMod; // Gebe das hier zusammengesetzte Array wieder zurück.
}
// Aufgerufen, wenn man auf den Monster-Button klickt.
// Der Spieler kämpft gegen das entsprechende Monster. Er erhält dann Erfahrungspunkte.
function fightMonster(_index) {
    console.log("Spieler kämpft gegen Monster: " + monsterArray[_index].monsterName + " und gewinnt!"); // Ohne Logik mit if/else ist so etwas wie ein Kampf nicht leicht umzusetzen.
    console.log("Spieler erhält: " + monsterArray[_index].monsterExperience + " Erfahrung!"); // Ohne Logik mit if/else ist so etwas wie ein Kampf nicht leicht umzusetzen.
